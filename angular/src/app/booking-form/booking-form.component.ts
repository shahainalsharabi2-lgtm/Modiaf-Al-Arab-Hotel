...
import { findRoomClassByRoomType } from '../utils/general-code-item.util';
import {
...
const RESERVATION_UI_FIELD_KEYS = [
  'account_Type',
  'season',
  'market_Code',
  'representatives',
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, RouterModule, LocaleNumberPipe, UiInlineTextComponent],
})
export class BookingFormComponent implements OnInit {
  ...
  private saveReservationAsCheckIn = false;
  ...
  /** حجز مسبق — جدول الدفعات */
  reservationPaymentLines: ReservationPaymentLine[] = [];
  reservationNotesOpen = false;
  ...
  private initFromRoute(): void {
    ...
    if (mode === 'transferRoom') {
      ...
      if (walkIn) {
        ...
        const roomNumber = this.route.snapshot.queryParamMap.get('roomNumber')?.trim();
        ...
        void this.router.navigate(['/front-desk/booking'], {
      ...
      if (!this.checkInMode) {
        ...
        if (bookingId > 0) {
          ...
          void this.router.navigate(['/front-desk/booking'], { replaceUrl: true });
  ...
  private loadGuestCodingOptions(): void {
    ...
    forkJoin({
      ...
      roomClasses: this.generalCodesService.getList('room-classes'),
    })
      .pipe(
        finalize(() => {
      ...
      .subscribe({
        next: ({ purposes, relationships, priceCodes, roomClasses }) => {
          ...
          this.roomClassOptions = this.sortGeneralCodeOptions(roomClasses);
  ...
  private enterReservationMode(confirmed = true, advanceDeposit = false): void {
    ...
    this.reservationPaymentLines = [];
    this.reservationNotesOpen = false;
    try {
    ...
    this.bookingForm.patchValue(
      { booking_Confirmed: confirmed, booking_Source: 'direct', departure_Time: '12:00' },
      { emitEvent: false },
    );
    this.syncBookingKindFromModes();
  ...
  onSubmit(): void {
    ...
    const saveReservationAsCheckIn = this.saveReservationAsCheckIn;
    this.saveReservationAsCheckIn = false;
    this.submitted = true;
    ...
    for (const key of RESERVATION_UI_FIELD_KEYS) {
    ...
    if (this.reservationMode) {
      bookingData.status = saveReservationAsCheckIn ? 'active' : 'reserved';
      ...
      bookingData.booking_Confirmed = saveReservationAsCheckIn
        ? true
        : bookingData.booking_Confirmed !== false;
      bookingData.booking_Source = 'direct';
    ...
    const notifyKind = this.addGuestBookingMode
      ? 'booking_add_guest' as const
      : saveReservationAsCheckIn || isCheckInFlow
        ? 'booking_checkin' as const
    ...
    this.bookingService.saveBooking(bookingData, { kind: notifyKind, params: bookingNotifyParams(bookingData) }).subscribe({
      next: () => {
        ...
        this.uiMsg.success(
          saveReservationAsCheckIn
            ? this.ui.screenText('booking', 'checkInSuccess')
            : this.reservationMode
            ? this.ui.screenText('booking', 'reservationSuccess')
            : isWalkInCheckIn
              ? this.ui.screenText('booking', 'checkInSuccess')
              : this.ui.screenText('booking', 'bookingCreatedSuccess'),
        );
        if ((!this.reservationMode || saveReservationAsCheckIn) && this.selectedRoom) {
  ...
  private submitExistingBookingUpdate(bookingData: Booking): void {
    ...
    const notify =
      this.transferRoomMode && this.initialRoomNumber
        ...
        : this.checkInMode && !this.extendStayMode
          ? { kind: 'booking_checkin' as const, params: bookingNotifyParams(updated) }
    ...
    this.bookingService.updateBooking(this.existingBookingId!, updated, notify).subscribe({
      next: () => {
        ...
        void this.router.navigate(['/front-desk'], {
          queryParams:
  ...
  selectedRoomClass(): GeneralCodeItem | null {
    return findRoomClassByRoomType(this.selectedRoom?.type, this.roomClassOptions);
  }

  walkInRoomClassCount(field: 'roomCount' | 'regularBedCount' | 'familyBedCount'): string {
    const value = this.selectedRoomClass()?.[field];
  ...
  reservationHotelName(): string {
    const name = this.hotelBranding.activeProfile().hotelName.trim();
    return name || this.ui.screenText('booking', 'nbHotelDefault');
  }

  reservationEmployeeName(): string {
    return this.walkInEmployeeName;
  }
  ...
  reservationRoomDetail(value: string | number | null | undefined): string {
    return this.walkInRoomDetail(value);
  }
  ...
  reservationRoomTotal(): number {
    const price = Number(this.selectedRoom?.price) || 0;
    const days = Math.max(1, Number(this.bookingForm.get('stay_Days')?.value) || 1);
    return Math.max(0, price * days);
  }

  reservationPaidTotal(): number {
    if (this.reservationPaymentLines.length) {
      return this.reservationPaymentLines.reduce((sum, line) => sum + (Number(line.amount) || 0), 0);
    }
    return Number(this.bookingForm.get('payment_Amount')?.value) || 0;
  }

  reservationRemainingTotal(): number {
    return Math.max(0, this.reservationRoomTotal() - this.reservationPaidTotal());
  }

  reservationCurrencyAmount(value: number): string {
    return `${this.priceCurrencySymbol()} ${value.toFixed(2)}`;
  }

  reservationRoomPriceText(): string {
    return this.reservationCurrencyAmount(Number(this.selectedRoom?.price) || 0);
  }

  reservationTotalText(): string {
    return this.reservationCurrencyAmount(this.reservationRoomTotal());
  }

  reservationPaidText(): string {
    return this.reservationCurrencyAmount(this.reservationPaidTotal());
  }

  reservationRemainingText(): string {
    return this.reservationCurrencyAmount(this.reservationRemainingTotal());
  }

  stepReservationField(controlName: string, delta: number, min: number): void {
    const ctrl = this.bookingForm.get(controlName);
    const current = Number(ctrl?.value) || 0;
    ctrl?.setValue(Math.max(min, current + delta));
    if (controlName === 'stay_Days') {
      this.onReservationNightsChange();
    }
    if (controlName === 'adults_Count' || controlName === 'children_Count') {
      this.syncPeopleCountFromOccupancy();
    }
    this.cdr.markForCheck();
  }

  onReservationArrivalChange(): void {
    this.cdr.markForCheck();
  }

  onReservationNightsChange(): void {
    this.calculateFinances();
    this.cdr.markForCheck();
  }

  onReservationBookingKindChange(): void {
    const raw = this.bookingForm.get('booking_Kind')?.value;
    if (!isBookingKindId(raw)) {
      return;
    }
    this.advanceDepositMode = raw === 'advance_deposit';
    if (this.advanceDepositMode) {
      this.applyAdvanceDepositValidators();
    } else {
      this.applyReservationValidators();
    }
    this.bookingForm.patchValue(
      { booking_Confirmed: raw !== 'unconfirmed' },
      { emitEvent: false },
    );
    this.cdr.markForCheck();
  }

  clearReservationField(controlName: string): void {
    this.bookingForm.patchValue({ [controlName]: '' }, { emitEvent: false });
    this.cdr.markForCheck();
  }

  addReservationPaymentLine(): void {
    this.reservationPaymentLines = [
      ...this.reservationPaymentLines,
      { amount: 0, currency: this.priceCurrencySymbol(), method: '' },
    ];
    this.syncReservationPaymentFromLines();
    this.cdr.markForCheck();
  }

  removeReservationPaymentLine(index: number): void {
    this.reservationPaymentLines = this.reservationPaymentLines.filter((_, i) => i !== index);
    this.syncReservationPaymentFromLines();
    this.cdr.markForCheck();
  }

  updateReservationPaymentLine(
    index: number,
    key: keyof ReservationPaymentLine,
    event: Event,
  ): void {
    const target = event.target as HTMLInputElement | HTMLSelectElement;
    const line = this.reservationPaymentLines[index];
    if (!line) {
      return;
    }
    if (key === 'amount') {
      line.amount = Number(target.value) || 0;
    } else if (key === 'currency') {
      line.currency = target.value;
    } else {
      line.method = target.value;
    }
    this.syncReservationPaymentFromLines();
    this.cdr.markForCheck();
  }

  syncReservationPaymentFromLines(): void {
    if (!this.reservationPaymentLines.length) {
      return;
    }
    const total = this.reservationPaymentLines.reduce(
      (sum, line) => sum + (Number(line.amount) || 0),
      0,
    );
    const method =
      this.reservationPaymentLines.find((line) => line.method.trim())?.method ?? '';
    this.bookingForm.patchValue(
      {
    ...
    if (this.advanceDepositMode && total > 0) {
      this.calculateFinances();
    }
  }

  toggleReservationNotes(): void {
    this.reservationNotesOpen = !this.reservationNotesOpen;
    this.cdr.markForCheck();
  }

  goWalkInCheckIn(): void {
    this.saveReservationAsCheckIn = true;
    this.onSubmit();
  }
}
