...
import {
  GENERAL_CODE_CATEGORIES_WITHOUT_FNAME,
  generalCodeShowsForeignName,
  generalCodeShowsDescription,
} from '../general-codes.constants';
import {
...
import {
  ...
  decodeCodeFieldDescription,
  encodeCodeFieldDescription,
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule, UiInlineTextComponent],
...
export class GeneralCodePanelComponent implements OnInit, OnChanges {
  ...
  readonly form = this.fb.group({
    ...
    foreignName: ['', Validators.maxLength(256)],
  ...
  get usesCodeField(): boolean {
    return GENERAL_CODE_CATEGORIES_WITHOUT_FNAME.has(this.category) && !this.isPrefCategory;
  }

  get showCodeForeignName(): boolean {
    return this.usesCodeField && !this.isPriceCodeType && !this.isRoomClasses;
  }

  get showModalDescription(): boolean {
    if (this.isPriceCodeType || this.isPrefCategory || this.isRoomClasses) {
      return false;
    }
    return this.showCodeForeignName || this.showDescription;
  }

  modalTitle(): string {
    const tab = this.ui.screenText('generalCodes', this.titleKey);
    const verbKey = this.editingId ? 'editVerb' : 'addVerb';
    const verb = this.ui.screenText('generalCodes', verbKey);
    return `${verb} ${tab}`;
  }
  ...
  itemCode(item: GeneralCodeItem): string {
    if (this.usesCodeField) {
      return (item.fName ?? '').trim() || '—';
    }
  ...
  toggleActionsMenu(itemId: string, event: Event): void {
    ...
    this.openActionsId = this.openActionsId === itemId ? null : itemId;
    this.cdr.markForCheck();
  }
  ...
  openEdit(item: GeneralCodeItem): void {
    ...
    if (this.isPrefCategory) {
      ...
      this.form.reset({
        ...
        countryDialCode: '',
        flagImageName: '',
        description: this.usesCodeField
          ? decodeCodeFieldDescription(item.description).notes
          : (item.description ?? ''),
        foreignName: this.usesCodeField
          ? decodeCodeFieldDescription(item.description).foreignName
          : '',
  ...
  private buildPayload(raw: ReturnType<typeof this.form.getRawValue>): CreateUpdateGeneralCodeItem {
    ...
    if (this.usesCodeField) {
      return {
        name: raw.name ?? '',
        fName: (raw.fName ?? '').trim() || null,
        description: encodeCodeFieldDescription(raw.foreignName ?? '', raw.description ?? ''),
        displayOrder: raw.displayOrder ?? 0,
      };
    }
