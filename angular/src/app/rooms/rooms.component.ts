...
import {
  distinctRoomClassCountsFromRooms,
  ROOM_LOCATION_UNSET_KEY,
  roomClassCountsForRoomType,
  roomLocationFilterValues,
  roomLocationGroupKey,
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule, RoomPreviewSheetComponent, UiInlineTextComponent],
})
export class RoomsComponent implements OnInit, AfterViewInit {
  ...
  devDataScope: 'all' | 'location' = 'all';
  devRoomType = '';
  devFilterRoomLocation = '';
  devFilterRoomCount = '';
  devFilterRegularBed = '';
  devFilterFamilyBed = '';
  devFilterPrice = '';
  ...
  /** الشريط السفلي — ينطوي أثناء تمرير بطاقات الغرف */
  devFooterCollapsed = false;

  /** الشريط العلوي — يُصغَّر إلى زر بحث عند التمرير، ويُفتح بالنقر */
  devTopChromeCompact = false;
  devTopChromePanelOpen = false;

  private readonly topChromeScrollState = createScrollChromeCompactState();
  ...
  private wireFilterToolbarScrollCollapse(): void {
    ...
    if (!this.roomsAdvancedLayout) {
      if (resetScrollChromeCompactState(this.topChromeScrollState)) {
        this.devTopChromeCompact = false;
        this.devTopChromePanelOpen = false;
      }
      this.devFooterCollapsed = false;
      this.cdr.markForCheck();
      return;
    }

    const targets = new Set<HTMLElement>();
    const appContentEl = this.hostEl.nativeElement.closest('.app-content') as HTMLElement | null;
    if (appContentEl) {
    ...
    const onScroll = () => {
      const scrollTop = readMaxScrollTop(this.hostEl.nativeElement);
      updateScrollChromeCompact(this.topChromeScrollState, scrollTop, (compact) => {
        if (this.devTopChromeCompact !== compact) {
          this.devTopChromeCompact = compact;
          if (compact) {
            this.devTopChromePanelOpen = false;
          }
          this.cdr.markForCheck();
      ...
      const collapsed = scrollTop > 48;
      if (this.devFooterCollapsed !== collapsed) {
        this.devFooterCollapsed = collapsed;
        this.cdr.markForCheck();
      }
    };

    onScroll();
    this.filterToolbarScrollSub = (targetScroll$ ? merge(windowScroll$, targetScroll$) : windowScroll$)
      .pipe(throttleTime(80))
      .subscribe(() => onScroll());
  }

  private syncDevFooterCollapsed(): void {
    if (!this.roomsAdvancedLayout) {
      if (this.devFooterCollapsed) {
        this.devFooterCollapsed = false;
        this.cdr.markForCheck();
      }
      if (resetScrollChromeCompactState(this.topChromeScrollState)) {
  ...
  get showTopChromePanel(): boolean {
    return !this.devTopChromeCompact || this.devTopChromePanelOpen;
  }

  get showTopChromePill(): boolean {
    return this.devTopChromeCompact && !this.devTopChromePanelOpen;
  }

  get topChromePillLabel(): string {
    if (this.devSearch.trim()) {
      return this.devSearch.trim();
    }
    if (this.smartFilterActive) {
      return this.ui.screenText('rooms', 'filterActivePill');
    }
    return this.ui.screenText('rooms', 'searchPlaceholder');
  }

  openTopChromePanel(): void {
    this.devTopChromePanelOpen = true;
    this.cdr.markForCheck();
    queueMicrotask(() => {
      const input = this.hostEl.nativeElement.querySelector(
        '#rooms-top-chrome-search',
      ) as HTMLInputElement | null;
      input?.focus();
    });
  }

  closeTopChromePanel(): void {
    this.devTopChromePanelOpen = false;
    this.cdr.markForCheck();
  }
  ...
  loadRooms(): void {
    ...
    forkJoin({
      ...
      roomClasses: this.generalCodesService.getList('room-classes').pipe(
        catchError((err) => {
          console.error('Error loading room classes for filters', err);
          return of([] as GeneralCodeItem[]);
        })
      ),
      roomLocations: this.generalCodesService.getList('room-locations').pipe(
        catchError((err) => {
          console.error('Error loading room locations for filters', err);
          return of([] as GeneralCodeItem[]);
        })
      ),
    }).subscribe({
      next: ({ rooms, bookings, roomClasses, roomLocations }) => {
        this.allBookings = bookings;
        this.rooms = rooms.map((r) => this.mapRoomFromApi(r as Room & Record<string, unknown>));
        this.roomClassOptions = roomClasses;
  ...
  /** غرفة فارغة (متاحة أو متسخة بدون نزيل) — قائمة ⋮ */
  isVacantRoomCard(room: Room): boolean {
    const scene = this.roomVisualScene(room);
    return scene === 'avail' || scene === 'dirty_vacant';
  }

  navigateVacantRoomBooking(room: Room): void {
    if (!this.isVacantRoomCard(room)) {
      return;
    }
    ...
    this.roomPreview = null;
    setPickedRoom(room);
    void this.router.navigate(['/front-desk'], { queryParams: { pmsTab: 'arriving' } });
  }

  navigateVacantRoomCheckIn(room: Room): void {
    if (!this.isVacantRoomCard(room)) {
      return;
    }
    const num = String(room.roomNumber ?? '').trim();
  ...
  roomLocationFilterOptions(): string[] {
    return roomLocationFilterValues(this.rooms, this.roomLocationOptions);
  }

  uniqueRoomClassRoomCounts(): number[] {
    return distinctRoomClassCountsFromRooms(this.rooms, this.roomClassOptions, 'roomCount');
  }

  uniqueRoomClassRegularBeds(): number[] {
    return distinctRoomClassCountsFromRooms(this.rooms, this.roomClassOptions, 'regularBedCount');
  }

  uniqueRoomClassFamilyBeds(): number[] {
    return distinctRoomClassCountsFromRooms(this.rooms, this.roomClassOptions, 'familyBedCount');
  }

  private roomMatchesClassCountFilters(room: Room): boolean {
    const counts = roomClassCountsForRoomType(room.type, this.roomClassOptions);
    if (this.devFilterRoomCount) {
      const target = Number(this.devFilterRoomCount);
      if (counts.roomCount !== target) {
        return false;
      }
    }
    if (this.devFilterRegularBed) {
      const target = Number(this.devFilterRegularBed);
      if (counts.regularBedCount !== target) {
        return false;
      }
    }
    if (this.devFilterFamilyBed) {
      const target = Number(this.devFilterFamilyBed);
      if (counts.familyBedCount !== target) {
        return false;
      }
    }
    return true;
  ...
  get roomClassCountFilterActive(): boolean {
    return !!(this.devFilterRoomCount || this.devFilterRegularBed || this.devFilterFamilyBed);
  }
  ...
  get smartFilterActive(): boolean {
    return !!(
      this.devRoomType ||
      this.devFilterRoomLocation ||
      this.roomClassCountFilterActive ||
      this.devFilterPrice ||
      this.devSearch.trim()
    );
  }
  ...
  clearSmartFilters(): void {
    this.devRoomType = '';
    this.devFilterRoomLocation = '';
    this.devFilterRoomCount = '';
    this.devFilterRegularBed = '';
    this.devFilterFamilyBed = '';
    this.devFilterPrice = '';
    this.devSearch = '';
    this.cdr.markForCheck();
  }
  ...
  filteredRoomsAdvanced(): Room[] {
    ...
    if (this.devFilterRoomLocation) {
      list = list.filter((r) =>
        storedGeneralCodeValueMatches(r.roomLocation, this.devFilterRoomLocation),
      );
    }
    if (this.roomClassCountFilterActive) {
      list = list.filter((r) => this.roomMatchesClassCountFilters(r));
    }
    ...
    if (this.devSearch.trim()) {
      ...
      list = list.filter(
        (r) =>
          ...
          (r.roomLocation && r.roomLocation.toLowerCase().includes(term)) ||
  ...
  locationsInAdvancedView(): string[] {
    const present = new Set(
      this.filteredRoomsAdvanced().map((room) => roomLocationGroupKey(room)),
    );
    const ordered = this.roomLocationFilterOptions().filter((loc) => present.has(loc));
    const extras = [...present].filter(
      (key) => key !== ROOM_LOCATION_UNSET_KEY && !ordered.includes(key),
    );
    extras.sort((a, b) => a.localeCompare(b, 'ar'));
    const list = [...ordered, ...extras];
    if (present.has(ROOM_LOCATION_UNSET_KEY)) {
      list.push(ROOM_LOCATION_UNSET_KEY);
    }
    return list;
  }

  roomsAtLocation(locationKey: string): Room[] {
    return this.filteredRoomsAdvanced().filter(
      (room) => roomLocationGroupKey(room) === locationKey,
    );
  }

  locationSectionLabel(locationKey: string, count: number): string {
    if (locationKey === ROOM_LOCATION_UNSET_KEY) {
      return this.ui
        .screenText('rooms', 'locationUnsetSection')
        .replace('{n}', String(count));
    }
    return this.ui
      .screenText('rooms', 'locationSection')
      .replace('{loc}', locationKey)
      .replace('{n}', String(count));
  }

  /** أقسام الشريط الأفقي: كل الغرف في شريط واحد أو تفصيل حسب موقع الغرفة */
  advancedSections(): { label: string; rooms: Room[] }[] {
    ...
    return this.locationsInAdvancedView().map((locationKey) => ({
      label: this.locationSectionLabel(locationKey, this.roomsAtLocation(locationKey).length),
      rooms: this.roomsAtLocation(locationKey),
    }));
  }
