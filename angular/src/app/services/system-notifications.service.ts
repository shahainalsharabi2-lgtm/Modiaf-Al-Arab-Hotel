...
const KIND_CHROME_KEY: Record<SystemNotificationKind, string> = {
  ...
  booking_checkin: 'notifyBookingCheckIn',
...
const KIND_ICON: Record<SystemNotificationKind, string> = {
  ...
  booking_checkin: 'fa-concierge-bell',
...
export class SystemNotificationsService {
  ...
  timeLabel(n: SystemNotification): string {
    return this.formatRelativeTime(n.at, 'short');
  }

  timeLabelPanel(n: SystemNotification): string {
    return this.formatRelativeTime(n.at, 'long');
  }

  categoryLabel(kind: SystemNotificationKind): string {
    if (kind.startsWith('room_')) {
      return this.ui.chromeLabel('notifyCategoryRoom');
    }
    return this.ui.chromeLabel('notifyCategoryReservation');
  }

  private formatRelativeTime(at: number, style: 'short' | 'long'): string {
    ...
    if (diffMin < 60) {
      const key = style === 'long' ? 'notifyTimeMinutesLong' : 'notifyTimeMinutes';
      return this.applyParams(this.ui.chromeLabel(key), { n: String(diffMin) });
    }
    const diffHr = Math.floor(diffMin / 60);
    if (diffHr < 24) {
      const key = style === 'long' ? 'notifyTimeHoursLong' : 'notifyTimeHours';
      return this.applyParams(this.ui.chromeLabel(key), { n: String(diffHr) });
    }
    const diffDay = Math.floor(diffHr / 24);
    const key = style === 'long' ? 'notifyTimeDaysLong' : 'notifyTimeDays';
    return this.applyParams(this.ui.chromeLabel(key), { n: String(diffDay) });
  }
