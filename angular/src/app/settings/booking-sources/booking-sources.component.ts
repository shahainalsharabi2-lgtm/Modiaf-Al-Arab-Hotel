...
import { BOOKING_SOURCES_SEED, BookingSourceRowDto } from './booking-sources.seed';
...
@Component({
  ...
  templateUrl: './booking-sources.component.html',
  styleUrls: ['../hotel-chains/hotel-chains.component.scss'],
})
export class BookingSourcesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.rows = BOOKING_SOURCES_SEED.map((row) => ({ ...row }));
  }
  ...
  filteredRows(): BookingSourceRowDto[] {
  ...
  openEdit(row: BookingSourceRowDto): void {
    if (!this.canEdit) {
      return;
    }
    void row;
  ...
  deleteRow(row: BookingSourceRowDto): void {
    if (!this.canEdit) {
      return;
    }
    this.openActionsId = null;
    void this.uiMsg.confirm(this.ui.screenText('settings', 'bookingSourcesDeleteConfirm')).then((ok) => {
      if (!ok) {
        return;
      }
