...
import { BOOKING_TYPES_SEED, BookingTypeRowDto } from './booking-types.seed';

type ViewMode = 'list' | 'grid';
type StatusFilter = 'all' | 'active' | 'inactive';
type SortKey = 'id' | 'code' | 'name' | 'displayOrder';
...
@Component({
  ...
  styleUrls: ['../hotel-chains/hotel-chains.component.scss', './booking-types.component.scss'],
})
export class BookingTypesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.rows = BOOKING_TYPES_SEED.map((row) => ({ ...row }));
  }
  ...
  filteredRows(): BookingTypeRowDto[] {
    ...
    if (q) {
      rows = rows.filter((r) => {
        const hay = `${r.id} ${r.code} ${r.name}`.toLowerCase();
  ...
  openEdit(row: BookingTypeRowDto): void {
    if (!this.canEdit) {
      return;
    }
    void row;
  ...
  deleteRow(row: BookingTypeRowDto): void {
    if (!this.canEdit) {
      return;
    }
    this.openActionsId = null;
    void this.uiMsg.confirm(this.ui.screenText('settings', 'bookingTypesDeleteConfirm')).then((ok) => {
      if (!ok) {
        return;
      }
