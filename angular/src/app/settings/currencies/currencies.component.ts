...
import { CURRENCIES_SEED, CurrencyRowDto } from './currencies.seed';
...
export class CurrenciesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.rows = CURRENCIES_SEED.map((row) => ({ ...row }));
  }
  ...
  openEdit(row: CurrencyRowDto): void {
    if (!this.canEdit) {
      return;
    }
    void row;
  ...
  deleteRow(row: CurrencyRowDto): void {
    if (!this.canEdit) {
      return;
    }
    this.openActionsId = null;
    void this.uiMsg.confirm(this.ui.screenText('settings', 'currenciesDeleteConfirm')).then((ok) => {
      if (!ok) {
        return;
      }
