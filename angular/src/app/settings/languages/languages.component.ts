...
import { LANGUAGES_SEED, LanguageRowDto } from './languages.seed';
...
export class LanguagesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.rows = LANGUAGES_SEED.map((row) => ({ ...row }));
  }
  ...
  openEdit(row: LanguageRowDto): void {
    if (!this.canEdit) {
      return;
    }
    void row;
  ...
  deleteRow(row: LanguageRowDto): void {
    if (!this.canEdit) {
      return;
    }
    this.openActionsId = null;
    void this.uiMsg.confirm(this.ui.screenText('settings', 'languagesDeleteConfirm')).then((ok) => {
      if (!ok) {
        return;
      }
