...
import { ROUTING_CODES_SEED, RoutingCodeRowDto } from './routing-codes.seed';
...
@Component({
  selector: 'app-routing-codes',
  standalone: true,
  imports: [CommonModule, FormsModule, UiInlineTextComponent],
  templateUrl: './routing-codes.component.html',
  styleUrls: ['../hotel-chains/hotel-chains.component.scss'],
})
export class RoutingCodesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.rows = ROUTING_CODES_SEED.map((row) => ({ ...row }));
  }
  ...
  filteredRows(): RoutingCodeRowDto[] {
  ...
  openEdit(row: RoutingCodeRowDto): void {
    if (!this.canEdit) {
      return;
    }
    void row;
  ...
  deleteRow(row: RoutingCodeRowDto): void {
    if (!this.canEdit) {
      return;
    }
    this.openActionsId = null;
    void this.uiMsg.confirm(this.ui.screenText('settings', 'routingCodesDeleteConfirm')).then((ok) => {
      if (!ok) {
        return;
      }
