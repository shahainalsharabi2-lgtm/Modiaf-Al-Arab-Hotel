...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, RouterLink, LocaleNumberPipe, UiInlineTextComponent],
  templateUrl: './room-preview-sheet.component.html',
  styleUrls: ['./room-preview-sheet.component.css'],
})
export class RoomPreviewSheetComponent implements OnInit {
  ...
  /** إخفاء زر التسكين عند فتح الورقة من مخطط الحجوزات */
  @Input() showWalkInCheckIn = true;
  ...
  ngOnInit(): void {
    this.generalCodes.getList('room-classes').subscribe({
      next: (items) => {
        this.roomClassOptions = (items ?? []).map((item) => applyRoomClassDescription(item));
      },
      error: () => {
  ...
  roomDetail(value: string | number | null | undefined): string {
    const text = String(value ?? '').trim();
    return text || '—';
  }

  roomClassFor(r: Room): GeneralCodeItem | null {
    return findRoomClassByRoomType(r.type, this.roomClassOptions);
  }

  roomClassLabel(r: Room): string {
    const cls = this.roomClassFor(r);
    if (!cls) {
      return this.ui.roomTypeLabel(r.type);
    }
    return (cls.name ?? '').trim() || this.ui.roomTypeLabel(r.type);
  }
  ...
  roomFeaturesText(r: Room): string {
    const summary = roomFeaturesSummary(parseRoomFeatures(r.roomFeatures), 8);
    return summary || '—';
  }

  canVacantActions(r: Room): boolean {
    return !this.booking && !this.pickRoomMode && (r.status === 'available' || r.status === 'dirty');
  }

  checkInQueryParams(r: Room): Record<string, string> {
    return {
      roomNumber: String(r.roomNumber ?? '').trim(),
    };
  }

  bookingQueryParams(r: Room): Record<string, string> {
    return { roomNumber: String(r.roomNumber ?? '').trim() };
  }
