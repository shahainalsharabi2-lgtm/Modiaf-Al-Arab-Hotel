...
export function roomClassCountsForRoomType(
  roomType: string | null | undefined,
  classes: GeneralCodeItem[],
): Pick<GeneralCodeItem, 'roomCount' | 'regularBedCount' | 'familyBedCount'> {
  const cls = findRoomClassByRoomType(roomType, classes);
  return {
    roomCount: cls?.roomCount ?? null,
    regularBedCount: cls?.regularBedCount ?? null,
    familyBedCount: cls?.familyBedCount ?? null,
  };
}

export function distinctRoomClassCountsFromRooms(
  rooms: ReadonlyArray<{ type?: string | null }>,
  classes: GeneralCodeItem[],
  field: 'roomCount' | 'regularBedCount' | 'familyBedCount',
): number[] {
  const set = new Set<number>();
  for (const room of rooms) {
    const value = roomClassCountsForRoomType(room.type, classes)[field];
...
export const ROOM_LOCATION_UNSET_KEY = '__unset__';

/** يطابق قيمة مخزّنة على الغرفة مع اختيار من المدخلات (اسم عربي أو أجنبي) */
export function storedGeneralCodeValueMatches(
  ...
  const lowerStored = storedKey.toLowerCase();
  const lowerSelected = selectedKey.toLowerCase();
  return storedKey === selectedKey || lowerStored === lowerSelected;
}

export function sortGeneralCodeItems(items: GeneralCodeItem[]): GeneralCodeItem[] {
  return [...items].sort(
    (a, b) =>
...
/** قيم مواقع الغرف للتصفية — من المدخلات أو من الغرف إن لم تُعرَّف مواقع */
export function roomLocationFilterValues(
  rooms: ReadonlyArray<{ roomLocation?: string | null }>,
  locationOptions: GeneralCodeItem[],
): string[] {
  const fromCodes = sortGeneralCodeItems(locationOptions)
  ...
  return [
    ...new Set(rooms.map((room) => (room.roomLocation ?? '').trim()).filter(Boolean)),
  ].sort((a, b) => a.localeCompare(b, 'ar'));
}

export function roomLocationGroupKey(room: { roomLocation?: string | null }): string {
  const loc = (room.roomLocation ?? '').trim();
  return loc || ROOM_LOCATION_UNSET_KEY;
}

/** يطابق نوع الغرفة المخزّن مع فئة من المدخلات (اسم عربي أو أجنبي) */
export function findRoomClassByRoomType(
  roomType: string | null | undefined,
  classes: GeneralCodeItem[],
): GeneralCodeItem | null {
...
/** يُخزَّن في Description للترميزات ذات الكود + الاسم الأجنبي + الوصف */
const CODE_FIELD_DESC_PREFIX = '__codeMeta__:';

export interface CodeFieldDescriptionMeta {
  foreignName: string;
  notes: string;
}

export function decodeCodeFieldDescription(description: string | null | undefined): CodeFieldDescriptionMeta {
  const raw = (description ?? '').trim();
  if (!raw) {
    return { foreignName: '', notes: '' };
  }
  if (raw.startsWith(CODE_FIELD_DESC_PREFIX)) {
    try {
      const parsed = JSON.parse(raw.slice(CODE_FIELD_DESC_PREFIX.length)) as {
        fn?: unknown;
        d?: unknown;
      };
      return {
        foreignName: String(parsed.fn ?? '').trim(),
        notes: String(parsed.d ?? '').trim(),
      };
    } catch {
      return { foreignName: '', notes: raw };
    }
  }
  return { foreignName: raw, notes: '' };
}

export function encodeCodeFieldDescription(foreignName: string, notes: string): string | null {
  const fn = foreignName.trim();
  const d = notes.trim();
  if (!fn && !d) {
    return null;
  }
  ...
  const payload = `${CODE_FIELD_DESC_PREFIX}${JSON.stringify({ fn, d })}`;
  return payload.length <= MAX_PREF_DESCRIPTION_LENGTH
    ? payload
    : payload.slice(0, MAX_PREF_DESCRIPTION_LENGTH);
}
