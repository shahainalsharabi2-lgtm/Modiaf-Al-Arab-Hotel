/**
 * يولّد PDF تفصيلي لملخص الباك-اند
 * التشغيل: node docs/generate-backend-summary-pdf.mjs
 */
...
const mdPath = join(__dirname, 'ملخص-الباك-اند.md');
const htmlPath = join(__dirname, 'backend-summary-ar.html');
const pdfPath = join(__dirname, 'Hotel-Backend-Summary-AR.pdf');
const pdfPathAr = join(__dirname, 'ملخص-الباك-اند.pdf');
...
<head>
  <meta charset="UTF-8"/>
  <title>ملخص الباك-اند — مضياف العرب للفنادق</title>
  <style>
