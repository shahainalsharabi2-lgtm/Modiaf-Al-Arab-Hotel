/**
 * PDF — شرح عملية تحديث قاعدة البيانات (فرونت + باك)
...
const mdPath = join(__dirname, 'شرح-تحديث-قاعدة-البيانات.md');
const htmlPath = join(__dirname, 'db-update-guide-ar.html');
const pdfPath = join(__dirname, 'Hotel-Database-Update-Guide-AR.pdf');
const pdfPathAr = join(__dirname, 'شرح-تحديث-قاعدة-البيانات.pdf');
...
<head>
  <meta charset="UTF-8"/>
  <title>شرح تحديث قاعدة البيانات — مضياف العرب</title>
  <style>
    ...
    h3 { color: #37474f; font-size: 10pt; page-break-after: avoid; }
    code {
      direction: ltr;
      unicode-bidi: embed;
    ...
    li { margin: 2px 0; }
    hr { border: none; border-top: 1px solid #b0bec5; margin: 12px 0; }
...
function main() {
  ...
  console.log('افتح HTML واطبع PDF:', htmlPath);
}
