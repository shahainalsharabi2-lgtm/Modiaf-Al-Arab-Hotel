/**
 * يولّد PDF لملخص العمليات وأسماء الأدوات
 * التشغيل: node docs/generate-operations-tools-pdf.mjs
 */
...
const mdPath = join(__dirname, 'ملخص-العمليات-والأدوات.md');
const htmlPath = join(__dirname, 'operations-tools-ar.html');
const pdfPath = join(__dirname, 'Hotel-Operations-Tools-AR.pdf');
const pdfPathAr = join(__dirname, 'ملخص-العمليات-والأدوات.pdf');
...
<head>
  <meta charset="UTF-8"/>
  <title>ملخص العمليات والأدوات — مضياف العرب للفنادق</title>
  <style>
    ...
    h3 { color: #37474f; font-size: 11pt; page-break-after: avoid; }
    code {
      direction: ltr;
      unicode-bidi: embed;
    ...
    li { margin: 2px 0; }
    hr { border: none; border-top: 1px solid #b0bec5; margin: 14px 0; }
    .meta { color: #546e7a; font-size: 9pt; }
    strong { color: #0d47a1; }
  </style>
</head>
<body>
